...
//    private static final Logger logger = LoggerFactory.getLogger(DynamicDataSourceRegister.class);
    ...
    public void registerBeanDefinitions(AnnotationMetadata annotationMetadata, BeanDefinitionRegistry beanDefinitionRegistry) {
        ...
        System.out.println("注册默认数据源成功");
//        logger.info("注册默认数据源成功");
        // 获取其他数据源配置
        defauleDataSourceProperties = binder.bind("spring.datasource.druid.slave", Map.class).get();
        String ts = evn.getProperty("spring.datasource.type");
        Class<? extends DataSource> clazz2 = getDataSourceType(ts);
        consumerDatasource = bind(clazz2, defauleDataSourceProperties);
        String key = "slaveDataSource";
        customDataSources.put(key, consumerDatasource);
        // 数据源上下文，用于管理数据源与记录已经注册的数据源key
        DynamicDataSourceContextHolder.dataSourceIds.add(key);
        System.out.println("注册数据源{}成功"+key);
...
//        }
        ...
        System.out.println("注册数据源成功，一共注册{}个数据源"+(customDataSources.keySet().size() + 1));
